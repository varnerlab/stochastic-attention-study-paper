% Restored supplementary material from the submitted arXiv version.
% Numerical results are retained for transparency and reinterpretation.
...
\section{Additional Submitted Experiments and Diagnostics}
\label{app:restored-experiments}

The experiments in this section are retained from the submitted manuscript for
scientific transparency, qualitative inspection, and reproducibility. Unless an
exact all-component row is shown in the main text, they characterize the
submitted finite-time, warm-started protocol rather than independent equilibrium
sampling. Novelty, diversity, and Hopfield energy are descriptive quantities;
they are not a cross-model quality ranking. Reported standard errors inherit the
submitted grouping convention and should not be interpreted as repeated-run
uncertainty.
...
\subsection{Submitted MNIST baseline record}
\label{app:submitted-mnist-table}
Table~\ref{tab:mnist-baselines} preserves the submitted numerical comparison so
that the revised exact-sampling analysis can be audited against the original
record. The columns describe endpoint geometry under different procedures.
They do not establish that one method is a better image generator.
\begin{table}[t]
\centering
\caption{Submitted MNIST digit-3 endpoint statistics ($K=100$, $d=784$).
Novelty, diversity, and Hopfield energy are descriptive geometry, not a quality
ranking. Values use the submitted mean $\pm$ SE grouping across 30 chains.
...
Digits 1 and 8 test whether the submitted endpoint behavior was specific to
digit 3. The images and tables are retained as descriptive sensitivity analyses.
...
\caption{Submitted MNIST digit-1 endpoint grids. The panels permit qualitative
...
\hfill
\makebox[0.19\textwidth]{\centering (e) ULA}
\caption{Submitted MNIST digit-8 endpoint grids under the same finite-time
...
\begin{table}[ht]
\centering
\caption{Submitted temperature spectrum for digit 8 ($K=100$, $d=784$,
$\alpha=0.01$, 30 chains). SNR is the per-step update ratio
$\sqrt{\alpha\beta/(2d)}$. It is a heuristic under spherical, unit-norm
assumptions, not an automatic regime boundary. Values use the submitted
mean $\pm$ SE grouping.}
...
\caption{Temperature spectrum for MNIST digit 8. Decreasing $\beta$ increases
the exact component width $\beta^{-1/2}$: endpoints broaden away from stored
patterns and eventually lose recognizable digit structure.}
...
$\beta=50$, shown after the submitted denoise readout; its nearest-memory labels
...
The following submitted table and figures document the full preprocessing and
decoder behavior. All comparisons use the same 68 sequences for fitting and
evaluation. They are therefore in-sample distributional diagnostics, and the
HMM pass rate is saturated across methods.
\begin{table}[ht]
\centering
\caption{Submitted PF00076 endpoint statistics ($K=68$, $L=71$, $d=59$
...
\end{minipage}
\caption{Submitted PF00076 diagnostics. Left: per-position amino-acid
frequencies for stored and ULA-decoded sequences. Right: nearest-memory
sequence-identity distributions. The figures describe the in-sample
mixture-plus-PCA-decoder pipeline and are not functional validation.}
\label{fig:protein}
\end{figure}
...
The finance experiment is retained as an exploratory application of finite-time
...
\begin{table}[ht]
\centering
\caption{Submitted finance summary. Distributional rows use an i.i.d. endpoint
...
The submitted Olivetti tables quantify memory-subset restriction. Hard masking
...
\begin{table}[ht]
\centering
\caption{Submitted subject-recovery rate on Olivetti faces ($K=100$,
...
\begin{table}[ht]
\centering
\caption{Submitted single-$\beta$ masked sweep on Olivetti subject 0, with
20 chains per row. Novelty is median minimum cosine distance to an in-subject
memory; hit rate uses the submitted nearest-mean rule.}
...
\section{Retained Baseline Method Specifications}\label{app:baseline-methods}
% ─────────────────────────────────────────────────────────────────────────────
...
submitted descriptive metrics. Smaller latent dimensions, larger KL weights,
...
Step-size, noise, and MALA comparisons are retained as diagnostics of the
finite-time transition kernel. High MALA acceptance is reported descriptively
and is not treated as proof that ULA has mixed or is unbiased. The submitted
one-step Gaussian perturbation used scale $\sqrt{2\alpha/\beta}$; the exact
component scale is $\beta^{-1/2}$, as evaluated in the revised main text.
\begin{table}[ht]
\centering
\caption{Submitted step-size sweep on MNIST digit 3 ($K=100$, $d=784$,
$\beta=2000$). ULA and MALA endpoint summaries are close at small $\alpha$ in
...
\begin{table}[ht]
\centering
\caption{Submitted Gaussian controls on MNIST digit 3 at $\beta=200$.
...
Gaussian sampling, which is reported in the revised main text.}
...
\caption{Submitted Gaussian-control images. Panels show stored patterns, ULA
...
\begin{table}[ht]
\centering
\caption{Submitted ULA and MALA endpoint summaries over $\beta$; $\Delta$
...
\section{Submitted VAE Data-Scale Sensitivity}
The following table is retained to document the submitted experiment. Because
...
Their concatenated output is mapped back by the specified linear reconstruction
\begin{equation}
...
The submitted proof-of-concept used disjoint blocks of PCA directions for the
$\mathbf P_h$. With $K=100$ digit-3 memories, the first 100 nonzero principal
directions were partitioned across $H$ heads; for $H=4$, each head received 25
directions. Table~\ref{tab:multihead} restores the reported sweep. The table is
descriptive: changing $H$ changes the product model and reconstruction, so it
is not an apples-to-apples claim that heads improve sampling from one fixed
target.

\begin{table}[H]
\centering
\caption{Submitted PCA-partitioned multi-head demonstration at $\beta=200$.
Metrics are computed against each configuration's own memory set.}
\label{tab:multihead}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
